...
- **双解歧义处理**：线∩圆、圆∩圆天然可能有两个交点。两个解都满足报文时，候选会同时列出（`(alt)` 后缀），用 **Next** 轮转、**Place** 落子、**Del** 忽略。约束近乎相切的低置信度解会以 `!` / 黄色标记提示（此时报文的整数度舍入会被几何放大，落点可能明显偏离）。
- **地图标记**：每个待处理候选在地图桌面上显示圆环 + 标杆 + `#n` 序号标签（与面板行一一对应），落子或忽略后自动消失。
