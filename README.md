...
由控制台旁的一排 3D 按钮驱动（准星点击）：

- **Survey（自动测绘解析）**：读取两台电报机纸带与笔记本的报文，自动识别 `名称 - 网格坐标` 参考点行与 `自<锚点>的方位角/距离` 约束行，按主题分组做几何解算（方位线/距离圆求交 + 多约束残差投票），解出敌方单位位置。主题解出后自动注册为新锚点，支持链式报文（如"自敌方炮兵指挥中心#2…"）。
- **多解歧义处理**：线∩圆、圆∩圆天然可能有两个交点。两个解都满足报文时，候选会同时列出（`(alt)` 后缀），用 **Next** 轮转、**Place** 落子、**Del** 忽略；**歧义会沿锚点链传播**——上游主题有双解时，下游主题对每个分支各解一次，真解不会因为上游抛硬币而丢失。越出地图边界的交点直接判为假解剪除。主题名唯一匹配友方实体时，以实体坐标为地面真值自动确认分支（敌方实体绝不读取，那是 Reveal 的活）。约束近乎相切的低置信度解以 `!` / 黄色标记提示。
- **地图标记**：每个待处理候选在地图桌面上显示圆环 + 标杆 + `#n` 序号标签（与面板行一一对应），落子或忽略后自动消失；解算不再成立的 `(alt)` 假设自动收回。
- **自动刷新**：默认每 3 秒检测情报文本变化，新报文自动重新解析、新目标自动冒泡；**Auto** 按钮可开关。重复解析按主题名去重，不会产生重复条目。
- **自动落子**：**Place** 把当前候选用对应类型的棋子放到解算位置（网格坐标 → 桌面坐标由全部实体图标的运行时变换做最小二乘仿射校准，逐任务自适应）。手动把棋子拖到候选附近（0.15km 内）也会自动识别为已落子。
- **铁巢自动归位**：报文的"铁巢"网格行是每个任务必报的真值，Survey 时自动把炮塔棋子摆到正确位置（玩家已手动摆好则不打扰）——它是 T1~T4 与 Fire 解算的基准。
- **直接开火**：**Fire** 不经过棋子，直接把当前候选的网格坐标经仿射换算后按 T 按钮同款公式生成射击诸元入队（双炮管调度），**不受棋子编号 1~4 的限制**，目标数无上限；入队后自动切到下一个候选，可连续点名。
- **Reveal（作弊）**：直接点亮全部地图实体的视觉，等价于全图已航拍揭示。

## 紧急转移与后勤车队（本 fork 新增）

- **转移检测**：紧急转移后炮位图标在地图上持续滑动，滑动开始即作废旧炮位锚点、暂停炮塔棋子自动摆放（滑动全程只通报一次，静止 3 秒判定结束）。
- **Convoy（自动车队征集，默认开）**：转移后报文给出新大格（"铁巢新位置位于X某处"）时，自动打"位置报告"卡征集车队情报：占用槽位的残留卡牌会被自动弹出，大格参数自动填入卡片的 Coordinate 控制台变量。每条车队报告（"相距Xkm：车队当前位置：格" / "车队发现铁巢：… NNN 自 格"）自动解析为几何约束，两条以上即对铁巢新位置做解算；唯一解确认后自动摆回炮塔棋子、恢复火控基准。最多自动打 4 次，同一大格失败后不再重试。
- **NestGPS（作弊，默认关）**：直接读炮位图标的运行时位置，逆仿射换算出精确网格坐标，跳过整个车队情报收集过程（与 Reveal 同性质，所以默认关）。
