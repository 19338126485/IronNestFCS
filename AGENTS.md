...
## 步兵支援请求（已实现，2026-08-13）

- 友军报点两种格式（解析期直接算出坐标，无几何解算）：
  "请求在我方阵地 M4 9:2 施放SMK弹…"（定点=阵地格）与
  "请求向方位角 227°、距我方阵地 H7 3:9 0.57km处发射HE弹…"（阵地格+方位/距离偏移）。
  长行被纸带换行截断时（"方位角 221°，距我方阵地 K7 0:5 0.89km"）弹种/期限丢失但地点可解。
- 候选名 `支援<弹种>@<大格>`；请求自带弹种时 Fire 优先用它（SMK 掩护/HE 杀伤）；
  "请于 HH:MM:SS 之前响应"为期限（战场时钟），过期 30s 后自动跳过。
